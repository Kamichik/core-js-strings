...
function orderAlphabetically(str) {
  let orderedStr = str;
  for (let j = 0; j < orderedStr.length; j += 1) {
    for (let i = 0; i < orderedStr.length - 1 - j; i += 1) {
      if (orderedStr.charAt(i) > orderedStr.charAt(i + 1)) {
        orderedStr =
          orderedStr.substring(0, i) +
          orderedStr.charAt(i + 1) +
          orderedStr.charAt(i) +
          orderedStr.substring(i + 2);
      }
    }
  }
  return orderedStr;
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (char === char.toLowerCase()) {
      invertedSentence += char.toUpperCase();
    } else {
      invertedSentence += char.toLowerCase();
    }
  }
